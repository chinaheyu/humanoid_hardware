    ...
    if(offline_service_init)
    {
        for(int i = 0; i < OFFLINE_MANAGE_MAX_SIZE; ++i)
        {
            if(offline_manage[i].enable == 0)
            {
                ...
                offline_manage[i].last_state = 0;
                offline_manage[i].last_time = 0;
                offline_manage[i].online_func = online_func;
                offline_manage[i].online_first_func = online_first_func;
                offline_manage[i].offline_func = offline_func;
                offline_manage[i].offline_first_func = offline_first_func;
...
void delete_offline_callback(int id)
{
    if(offline_service_init && id > -1 && id < OFFLINE_MANAGE_MAX_SIZE)
    {
        ...
        offline_manage[id].last_state = 0;
        offline_manage[id].online_func = NULL;
        offline_manage[id].online_first_func = NULL;
        offline_manage[id].offline_func = NULL;
        offline_manage[id].offline_first_func = NULL;
        offline_manage[id].last_time = 0;
...
void offline_service(void *argument)
{
    ...
    for(int i = 0; i < OFFLINE_MANAGE_MAX_SIZE; ++i)
    {
        if(offline_manage[i].enable)
        {
            ...
            {
                if(offline_manage[i].last_state == 1)
                {
                    offline_manage[i].last_state = 0;
                    
                    if(offline_manage[i].offline_first_func != NULL)
                    {
                        offline_manage[i].offline_first_func();
                    }
                }
            ...
            {
                if(offline_manage[i].last_state == 0)
                {
                    offline_manage[i].last_state = 1;
                    
                    if(offline_manage[i].online_first_func != NULL)
                    {
                        offline_manage[i].online_first_func();
                    }
                }
